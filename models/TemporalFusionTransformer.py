...
class TFTEmbedding(nn.Module):
    def __init__(self, configs):
        ...
        self.observed_embedding = nn.ModuleList([DataEmbedding(
            1, configs.d_model, dropout=configs.dropout) for _ in range(self.observed_len)])
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        if self.static_len:
            ...
            static_input = torch.stack([embed(x_enc[:, :1, self.static_pos[i]].unsqueeze(
                -1), None).squeeze(1) for i, embed in enumerate(self.static_embedding)], dim=-2)
        else:
            static_input = None

        # observed_input: [B,T,C,d_model]
        observed_input = torch.stack([embed(x_enc[:, :, self.observed_pos[i]].unsqueeze(
            -1), None) for i, embed in enumerate(self.observed_embedding)], dim=-2)

        x_mark = torch.cat(
            [x_mark_enc, x_mark_dec[:, -self.pred_len:, :]], dim=-2)
...
class GateAddNorm(nn.Module):
    def __init__(self, input_size, output_size):
        ...
        self.projection = nn.Linear(
            input_size, output_size) if input_size != output_size else nn.Identity()
...
class GRN(nn.Module):
    def __init__(self, input_size, output_size, hidden_size=None, context_size=None, dropout=0.0):
        ...
        self.lin_c = nn.Linear(
            context_size, hidden_size) if context_size is not None else None
        self.lin_i = nn.Linear(hidden_size, hidden_size)
        self.dropout = nn.Dropout(dropout)
        self.project_a = nn.Linear(
            input_size, hidden_size) if hidden_size != input_size else nn.Identity()
...
class VariableSelectionNetwork(nn.Module):
    def __init__(self, d_model, variable_num, dropout=0.0):
        ...
        self.joint_grn = GRN(d_model * variable_num, variable_num,
    ...
    def forward(self, x: Tensor, context: Optional[Tensor] = None):
        ...
        selection_result = torch.matmul(
            x_processed, selection_weights.unsqueeze(-1)).squeeze(-1)
...
class StaticCovariateEncoder(nn.Module):
    def __init__(self, d_model, static_len, dropout=0.0):
        ...
        self.static_vsn = VariableSelectionNetwork(
            d_model, static_len) if static_len else None
...
class InterpretableMultiHeadAttention(nn.Module):
    def __init__(self, configs):
        ...
        self.out_projection = nn.Linear(
            self.d_head, configs.d_model, bias=False)
        self.out_dropout = nn.Dropout(configs.dropout)
        self.scale = self.d_head ** -0.5
        example_len = configs.seq_len + configs.pred_len
        self.register_buffer("mask", torch.triu(torch.full(
            (example_len, example_len), float('-inf')), 1))

    def forward(self, x):
        # Q,K,V are all from x
        B, T, d_model = x.shape
        qkv = self.qkv_linears(x)
        q, k, v = qkv.split((self.n_heads * self.d_head,
                            self.n_heads * self.d_head, self.d_head), dim=-1)
        ...
        attention_score = torch.matmul(
            q.permute((0, 2, 1, 3)), k.permute((0, 2, 3, 1)))  # [B,n,T,T]
        ...
        attention_out = torch.matmul(
            attention_prob, v.unsqueeze(1))  # [B,n,T,d]
...
class TemporalFusionDecoder(nn.Module):
    def __init__(self, configs):
        ...
        self.enrichment_grn = GRN(configs.d_model, configs.d_model,
                                  context_size=configs.d_model, dropout=configs.dropout)
        self.attention = InterpretableMultiHeadAttention(configs)
        self.gate_after_attention = GateAddNorm(
    ...
    def forward(self, history_input, future_input, c_c, c_h, c_e):
        ...
        # c_c, c_h, c_e: [B,d]
        # LSTM
        c = (c_c.unsqueeze(0), c_h.unsqueeze(0)
             ) if c_c is not None and c_h is not None else None
        ...
        # Static enrichment
        enriched_features = self.enrichment_grn(
            temporal_features, c_e)  # [B,T,d]
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast':
            dec_out = self.forecast(
                x_enc, x_mark_enc, x_dec, x_mark_dec)  # [B,pred_len,C]
