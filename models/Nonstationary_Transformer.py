...
class Projector(nn.Module):
    ...
    def __init__(self, enc_in, seq_len, hidden_dims, hidden_layers, output_dim, kernel_size=3):
        ...
        for i in range(hidden_layers - 1):
            layers += [nn.Linear(hidden_dims[i],
                                 hidden_dims[i + 1]), nn.ReLU()]
...
class Model(nn.Module):
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        std_enc = torch.sqrt(torch.var(
            # B x 1 x E
            x_enc, dim=1, keepdim=True, unbiased=False) + 1e-5).detach()
        x_enc = x_enc / std_enc
        ...
        dec_out = self.decoder(dec_out, enc_out, x_mask=None,
                               cross_mask=None, tau=tau, delta=delta)
    ...
    def imputation(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask):
        ...
        std_enc = torch.sqrt(torch.sum(x_enc * x_enc, dim=1) /
    ...
    def classification(self, x_enc, x_mark_enc):
        ...
        std_enc = torch.sqrt(
            # B x 1 x E
            torch.var(x_enc - mean_enc, dim=1, keepdim=True, unbiased=False) + 1e-5).detach()
        # B x S x E, B x 1 x E -> B x 1, positive scalar
        tau = self.tau_learner(x_raw, std_enc).exp()
        # B x S x E, B x 1 x E -> B x S
        delta = self.delta_learner(x_raw, mean_enc)
