...
class TokenMixer(nn.Module):
    def __init__(self, input_seq=[], batch_size=[], channel=[], pred_seq=[], dropout=[], factor=[], d_model=[]):
        ...
        self.layers = nn.Sequential(nn.Linear(self.input_seq, self.pred_seq * self.factor),
                                    ...
                                    nn.Linear(self.pred_seq *
                                              self.factor, self.pred_seq)
                                    )
...
class ResolutionBranch(nn.Module):
    def __init__(self,
        ...
        self.patch_num = int(
            (self.input_seq - self.patch_len) / self.patch_stride + 2)

        self.patch_norm = nn.BatchNorm2d(self.channel)
        self.patch_embedding_layer = nn.Linear(
            self.patch_len, self.d_model)  # shared among all channels
...
class Model(nn.Module):
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, batch_y_mark):
        ...
        # De-Normalization
        dec_out = pred * \
            (stdev[:, 0].unsqueeze(1).repeat(1, self.args.pred_len, 1))
        dec_out = dec_out + \
            (means[:, 0].unsqueeze(1).repeat(1, self.args.pred_len, 1))
        return dec_out
