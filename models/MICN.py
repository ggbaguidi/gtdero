...
class MIC(nn.Module):
    ...
    def __init__(self, feature_size=512, n_heads=8, dropout=0.05, decomp_kernel=[32], conv_kernel=[24],
        ...
        # feedforward network
        self.conv1 = nn.Conv1d(in_channels=feature_size,
                               out_channels=feature_size * 4, kernel_size=1)
        self.conv2 = nn.Conv1d(in_channels=feature_size * 4,
                               out_channels=feature_size, kernel_size=1)
        ...
        for ii in conv_kernel:
            if ii % 2 == 0:  # the kernel of decomposition operation must be odd
                ...
                isometric_kernel.append(
                    (configs.seq_len + configs.pred_len + ii) // ii)
            else:
